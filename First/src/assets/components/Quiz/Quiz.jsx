...
import ProgressBar from "../ProgressBar/progessBar.jsx";
...
const Quiz = ({ isDarkMode }) => {
  ...
  const handleKeyDown = (event) => {
    switch (event.key) {
      case "Enter":
        if (selectedSubject === null) {
          handleSubjectSelect(event.target.getAttribute("data-index"));
        } else if (showSubmit) {
          checkAnswer();
        }
        break;
      case "ArrowUp":
        if (selectedSubject === null) {
          ...
          const prevIndex = Math.max(selectedSubject - 1, 0);
          setSelectedSubject(prevIndex);
        } else if (!isOptionDisabled) {
          // Move to previous option
          const prevIndex = Math.max(selectedOption - 1, 0);
      ...
      case "ArrowDown":
        if (selectedSubject === null) {
          ...
          const nextIndex = Math.min(selectedSubject + 1, 3); // Assuming 4 subjects
          setSelectedSubject(nextIndex);
        } else if (!isOptionDisabled) {
          // Move to next option
          const nextIndex = Math.min(selectedOption + 1, 3); // Assuming 4 options
